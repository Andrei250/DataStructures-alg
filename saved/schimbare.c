...
int checkForCycle(int from, int **Graph, int nod,
	...
	for (int i = 1; i <= N; ++i)
		if (Graph[nod][i] && (i != nod)) {
			...
			if (!visited[i]) {
				if (checkForCycle(nod, Graph, i, visited,
					FStack, N, &(*number), top))
					return 1;
			}
		}
	...
	while (Q) {
		...
		for (int i = 1; i <= N; ++i)
			if (i != node && !visited[i] &&
				...
				// daca parintele este acelasi cu parintele
				// nodului curent atunci sar un pas pentru ca
				// s-ar creea un infinite loop
				...
				// scad costurile parintelui, deoarece fiecare
				// nod are un cost deja calculat de la parintele
				// din arbore si doresc sa calculez exact costul
				// ciclului si sa aflu valoarea doar cu
				// muchiile din ciclu tin cont ca in nodul i
				// costurile se inverseaza fata de
...
int main(void) {
	...
	char *p = strtok_r(string, " ", &saveptr);
	int N = atoi(p);
